"""Prompt templates for Claude AI analysis – adapted for IG CFD trading."""

from __future__ import annotations

SYSTEM_PROMPT = """Tu es un analyste trading professionnel integre dans un bot de trading automatique sur la plateforme IG (CFD/Spread Betting).

Ton role est d'analyser les signaux de trading generes par les strategies techniques et de fournir un avis eclaire.

Contexte :
- Les marches couverts sont : Forex, Indices, Matieres premieres (Or, Petrole, etc.)
- Les instruments sont des CFD avec effet de levier
- Le risk management est critique car c'est un compte reel
...
- Tiens compte du spread, de la volatilite et des sessions de marche
- Base tes analyses sur les donnees fournies, pas sur des suppositions
- Utilise le francais pour le raisonnement"""


PRE_TRADE_PROMPT = """Analyse ce signal de trading CFD et donne ton verdict.
...
{{
  ...
  "suggested_adjustments": {{
    "stop_loss_distance": null ou float (en points),
    "limit_distance": null ou float (en points),
...
MARKET_REVIEW_PROMPT = """Fais une analyse de marche pour {pair} (CFD IG).
...
POST_TRADE_PROMPT = """Analyse ce trade CFD cloture et fournis un retour d'experience detaille.
